...
这里用我做写匠的例子，校对功能对文字检查需要做到什么颗粒度，PM 要根据需求和产品的定位，来衡量并制定对错误的边界，需要 PM 在实际项目中认真思考和分析。比如：
...
分解需求到执行细节，也是 AI PM 们在工作中很重要的能力，这里需要理解不同的问题，有哪些解决方案，以及需要哪些合适的人来做这件事。

这里我举个现在工作的例子，比如为了算法迭代需求是提高召回率，就需要扩大校对规则库规模，根据这个目标，AI PM 将需求分解到每个人要做的事情，以及时间控制。
...
然后是对产品在开发周期的控制问题。首先 AI PM 理解需求背后的用户场景和客户目标，然后将需求拆解，需要哪些部门的帮助和争取哪些资源，然后拆解任务流程，对不同部门和需求的关系梳理，排需求优先级，...有时候 PM 要异步同时处理多个问题，要控制自己的时间分配，在什么阶段做什么事，哪些必须提前做，哪些可以后置，目的都是保证产品的进展，按时交付。
...
大部分的 PM 其实不需要了解这些，AI PM 工作中对比较底层的算法 PM 来说会直接相关。

入门 AI PM，最简单就是数据标注，可以通过数据来理解基本的算法原理和常识，PM 从项目具体行动中去做，去学，才能理解背后的规则的。
