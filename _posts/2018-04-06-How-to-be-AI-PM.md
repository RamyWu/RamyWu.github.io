...
 一般来说，toC 产品的整个业务系统并不是全都是 AI 来实现，只是有些问题，可以通过 AI 的技术来解决。如果中小公司的资源不足或技术门槛过高解决不了这个问题，就可能需要找第三方的 AI 服务合作，这就是乙方 toB 场景的 AI 服务了：

> 比如我之前在商汤做的金融场景解决方案中有一个是活体检测的 API/SDK->用于解决客户的在线用户身份验证、提升安全性、比人工更快速便捷、减轻客服人力资源的需求，给客户提供的API/SDK，对技术的要求是很高的，客户关心算法准确率、召回率、速度、体积、还有内存占用、API抗压性等，都是 这类的 AI PM 需要考虑到，和工程团队和算法团队共同解决的问题，甚至每天都在解决这些问题。除了人脸，还有其他场景，比如语音识别...也是类似，这块对乙方--AI 公司来说就是做的 toB 服务，对甲方--目标客户来说这是他们业务种的一个流程或是某一个子功能。

 
用 AI 能力区分的话，toC 公司可能还分为两种：

1. 大部分的toC产品，只有一部分业务流程种的某一个模块必须要用到 AI技术，比如抖音的尬舞机、faceU的人脸贴纸、支付宝的身份验证...（也许不久后就会有更多模块和流程都能 AI 实现了），这类C端产品的 PM 我们称为常规 PM，这些产品使用的AI服务基本都是从第三方AI合作来解决自己的需求，因为成本较高技术门槛也很大，该类公司不会自主研发。
2. 还有一种toC产品，AI 技术本身就是它的核心业务，比如我现在做的写匠产品（当然写匠也可以做成 toB 服务，完全是看我们的战略定位）和英文写作工具竞品 grammarly，核心竞争力在 AI NLP 上，技术不能交给第三方来做，不论门槛多高或者是技术资源不足，都需要靠自己来研发了，这类公司的 AI PM 要学习的东西就很多了，既要了解产品本身的场景需求，又要思考算法提供的技术能力如何无缝衔接到产品的核心功能里，如果算法能力明显跟不上用户需求，比如写匠的校对功能的准确率（找到错别字是不是误报的）、召回率（全文找到的错别字个数是不是少了）过低，那这款校对工具给用户带来是帮助，还是浪费用户的时间？...为了提升这些指标，要花很大功夫的，算法能力提升不了，产品就一直卡在这里没法走下去。
...
我做写匠，和计算机视觉领域的还不太一样，NLP 的难度很大，许多技术还没有突破，产品本身又牵扯到语言学领域的问题，对 PM 初学者来说水很深。

比如我在早期（入职0-6个月）会有这些工作：

- 理解写匠产品本身的需求场景、做同类竞品分析和测试...，
- 进行一些用户调研、访谈、...
- 写匠产品本身核心竞争力，反馈给用户的各种写作建议，如：基于认知科学的xx原理、涉及到的某些文学属性和评价指标、语言风格、...
- 根据任务从网上获取语料库，建立通用语料库，专用语料库、各行业的词表...
- 处理数据集，数据整理，...
- 算法评测相关工作，比如，制定算法评测指标，进行评测集标注，做算法评测，分析结果...
- 产品从0-1终于开始进入到工程开发阶段，原型设计、交互设计、项目管理、前端测试、申请各项第三方服务...
...
上述工作中，对我来说最难的地方是如何制定评价标准的问题，怎么把这些文学方面的东西变成硬性的指标，也就是算法如何建模的问题。如果底层的问题没想清楚或者是解决不了，即使把代码写好了，模型训练好了，最后用户也会觉得怎么那么硬邦邦的，很奇怪，就觉得这产品很不实用。

除了建模问题，还有语料库获取、数据处理、算法接入工程的各个环节也会有许多技术难题，这里不多赘述。



比较容易跑偏的是，最开始我没有理解 AI PM 真正的角色定义。
...
所以我们又面临了研发周期很长的问题，研发看不到尽头，大家投入了很多时间也不一定真的能够解决这些问题。即使定位明确，需求存在，但是这些难题短期 AI 解决不了，或者解决的程度有限。所以整个 NLP 行业，大家也都处于探索和尝试。

在不同时期，都要做竞品分析，我做写匠的设计之前，从用户群体、产品结构框架、逻辑、交互，到他们的算法能力->比如竞品所提供的语法检查范围...深入到各种细节的对比，慢慢尝试分析竞品为什么要做这样的设计，基于什么样的理由。



## 如何把产品经理已有的能力迁移到实际项目中？
...
因为光分析别家产品的东西，不去思考，永远都是自己的主观臆断。自己做出来的，或者自己从解决一个问题出发，而找到的东西，才能有更强的理解。
